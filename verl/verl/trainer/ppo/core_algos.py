...
def agg_loss(loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str):
    ...
    elif loss_agg_mode == "seq-mean-token-sum-norm-fcp":
        print(f"[DEBUG] Using seq-mean-token-sum-norm-fcp loss_agg_mode...")
        seq_losses = torch.sum(loss_mat * loss_mask, dim=-1)
        loss = torch.sum(seq_losses) / 500
    else:
...
def compute_policy_loss_sft(
    ...
    # print("[DEBUG] USING SFT POLICY LOSS !!!" + "*"*100)

    nll = -log_prob  # (B, T)

    sft_loss = agg_loss(loss_mat=nll, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
